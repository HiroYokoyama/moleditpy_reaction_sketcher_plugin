...
def create_reaction_icon(tool_name, size=32):
    ...
    if tool_name == "select":
        ...
        head = 5
        painter.drawLine(
            QPointF(size - inner_margin, y1),
            QPointF(size - inner_margin - head, y1 - head * 0.7),
        )
        painter.drawLine(
            QPointF(inner_margin, y2), QPointF(inner_margin + head, y2 + head * 0.7)
        )
        ...
        painter.drawPolygon(
            QPolygonF(
                [
                    QPointF(inner_margin, y),
                    QPointF(inner_margin + head, y - head * 0.6),
                    QPointF(inner_margin + head, y + head * 0.6),
                ]
            )
        )
        painter.drawPolygon(
            QPolygonF(
                [
                    QPointF(size - inner_margin, y),
                    QPointF(size - inner_margin - head, y - head * 0.6),
                    QPointF(size - inner_margin - head, y + head * 0.6),
                ]
            )
        )
        ...
        y1, y2 = c - 2.5, c + 2.5
        painter.drawLine(
            QPointF(inner_margin, y1), QPointF(size - inner_margin - 4, y1)
        )
        painter.drawLine(
            QPointF(inner_margin, y2), QPointF(size - inner_margin - 4, y2)
        )
        head = 8
        ...
        painter.drawPolygon(
            QPolygonF(
                [
                    ...
                    QPointF(end.x() - head, end.y() - head * 0.7),
                    QPointF(end.x() - head, end.y() + head * 0.7),
                ]
            )
        )
        ...
        head = 5
        painter.drawLine(
            QPointF(size - inner_margin, c),
            QPointF(size - inner_margin - head, c - head * 0.7),
        )
        painter.drawLine(
            QPointF(size - inner_margin, c),
            QPointF(size - inner_margin - head, c + head * 0.7),
        ...
        bw = 5
        painter.drawLine(
            QPointF(inner_margin + bw, inner_margin),
            QPointF(inner_margin, inner_margin),
        )
        painter.drawLine(
            QPointF(inner_margin, inner_margin),
            QPointF(inner_margin, size - inner_margin),
        )
        painter.drawLine(
            QPointF(inner_margin, size - inner_margin),
            QPointF(inner_margin + bw, size - inner_margin),
        )
        painter.drawLine(
            QPointF(size - inner_margin - bw, inner_margin),
            QPointF(size - inner_margin, inner_margin),
        )
        painter.drawLine(
            QPointF(size - inner_margin, inner_margin),
            QPointF(size - inner_margin, size - inner_margin),
        )
        painter.drawLine(
            QPointF(size - inner_margin, size - inner_margin),
            QPointF(size - inner_margin - bw, size - inner_margin),
        )
        ...
        painter.drawText(
            QRectF(0, size * 0.18, size - 7, size),
        ...
        painter.drawText(
            QRectF(size * 0.3, size * 0.18, size * 0.3, size),
            Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter,
            "2",
        )
        ...
        painter.setFont(f_main)
        painter.drawText(
            QRectF(size * 0.5, 0, size * 0.4, size),
            Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter,
            "O",
        )
...
def create_style_icon(item_type, style_name, selected=False):
    ...
    if item_type == "arrow_no":
        ...
        if style_name == "triangle":
            ...
            painter.setPen(
                QPen(
                    QColor("#005a9e"), 2, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap
                )
            )
            p1 = QLineF.fromPolar(head_len, angle + 180 + 35).p2() + end
